...
func newSelfServiceBrandingIosUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  "statusBarTextColor": "dark"
	...
	_ = cmd.RegisterFlagCompletionFunc("set", func(_ *cobra.Command, _ []string, _ string) ([]string, cobra.ShellCompDirective) {
		return []string{
			"brandingName=", "brandingNameColorCode=", "headerBackgroundColorCode=", "iconId=", "menuIconColorCode=", "statusBarTextColor=",
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						var rid string
						if rid == "" {
							id, err := resolveNameToIDForApply(reqCtx, ctx.Client, "/v1/self-service/branding/ios", "brandingName", "id", entry, noInputBulk)
							if err != nil {
				...
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d self-service-branding-ios. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				return batchDeleteError(cmd, okCount, failCount, firstErr, "self-service-branding-ios deletes")
			}

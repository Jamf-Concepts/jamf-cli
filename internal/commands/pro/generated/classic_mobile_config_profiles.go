...
func NewClassicMobileConfigProfilesCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd.AddCommand(newClassicMobileConfigProfilesDeleteByNameCmd(ctx))
...
func newClassicMobileConfigProfilesDeleteByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
	)
	...
	cmd := &cobra.Command{
		...
		Short: "Delete a mobile_device_configuration_profile by name",
		Example: `  # Delete a mobile_device_configuration_profile by name (with confirmation)
  jamf-cli classic-mobile-config-profiles delete-by-name "Example"

  # Delete without confirmation prompt
  jamf-cli classic-mobile-config-profiles delete-by-name "Example" --yes`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			name := args[0]
			...
			if id == "" {
				return fmt.Errorf("no mobile_device_configuration_profile found with name %q", name)
			}

			if flagDryRun {
				fmt.Fprintf(os.Stderr, "[dry-run] Would delete mobile_device_configuration_profile %q (id: %s)\n", name, id)
				return nil
			}
			if !flagYes {
				if noInput {
				...
				fmt.Fprintf(os.Stderr, "This will delete mobile_device_configuration_profile %q (id: %s). Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			path := fmt.Sprintf("/JSSResource/mobiledeviceconfigurationprofiles/id/%s", url.PathEscape(id))
			resp, err := ctx.Client.Do(reqCtx, "DELETE", path, nil)
			if err != nil {
				return err
			}
			...
			if resp.StatusCode == http.StatusNoContent || resp.StatusCode == http.StatusOK {
				fmt.Fprintf(os.Stderr, "Deleted mobile_device_configuration_profile %q (id: %s)\n", name, id)
				return nil
			}

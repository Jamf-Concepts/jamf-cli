...
// NewSelfServicePlusSettingsCmd creates the self-service-plus settings command group
func NewSelfServicePlusSettingsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "settings",
		Short:       "Manage self-service-plus settings",
		Long:        `Manage self-service-plus settings in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newSelfServicePlusSettingsGetCmd(ctx))
	cmd.AddCommand(newSelfServicePlusSettingsUpdateCmd(ctx))

	return cmd
}

func newSelfServicePlusSettingsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get self-service-plus settings
  jamf-cli pro self-service-plus settings get

  # Get self-service-plus settings and output as YAML
  jamf-cli pro self-service-plus settings get -o yaml`,
...
func newSelfServicePlusSettingsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
		flagSet      []string
	)
  ...
  jamf-cli pro self-service-plus settings update --set field=value

  # Replace self-service-plus settings from a full JSON document
  jamf-cli pro self-service-plus settings get -o json | jq '.field = "value"' | jamf-cli pro self-service-plus settings update

  # Update from a file
  jamf-cli pro self-service-plus settings update --from-file self-service-plus-settings.json`,

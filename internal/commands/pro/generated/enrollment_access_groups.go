...
// NewEnrollmentAccessGroupsCmd creates the enrollment-access-groups command group
func NewEnrollmentAccessGroupsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "enrollment-access-groups",
		Short:       "Manage enrollment-access-groups",
		Long:        `Manage enrollment-access-groups in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newEnrollmentAccessGroupsListCmd(ctx))
	cmd.AddCommand(newEnrollmentAccessGroupsGetCmd(ctx))
	cmd.AddCommand(newEnrollmentAccessGroupsCreateCmd(ctx))
	cmd.AddCommand(newEnrollmentAccessGroupsUpdateCmd(ctx))
	cmd.AddCommand(newEnrollmentAccessGroupsDeleteCmd(ctx))
	cmd.AddCommand(newEnrollmentAccessGroupsApplyCmd(ctx))

	return cmd
}

func newEnrollmentAccessGroupsListCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage                int
		flagPageSize            int
		flagSort                []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all enrollment-access-groups
  jamf-cli pro enrollment-access-groups list

  # List enrollment-access-groups and extract IDs
  jamf-cli pro enrollment-access-groups list --field id`,
...
func newEnrollmentAccessGroupsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
	...
	cmd := &cobra.Command{
		...
		Short: "Retrieve the configured LDAP groups configured for User-Initiated Enrollment",
		Long:  "Retrieves the configured LDAP groups configured for User-Initiated Enrollment.",
		Example: `  # Get a enrollment-access-group by ID
  jamf-cli pro enrollment-access-groups get 1

  # Get a enrollment-access-group by name
  jamf-cli pro enrollment-access-groups get --name "Example"

  # Get a enrollment-access-group and output as YAML
  jamf-cli pro enrollment-access-groups get 1 -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "Read User-Initiated Enrollment", "jamf:api": "pro", "jamf:gateway-privileges": "user-initiated-enrollment:read"},
...
func newEnrollmentAccessGroupsCreateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Show the JSON template for creating a enrollment-access-group
  jamf-cli pro enrollment-access-groups create --scaffold

  # Create a enrollment-access-group from JSON
  echo '{"name":"Example"}' | jamf-cli pro enrollment-access-groups create

  # Get a enrollment-access-group, modify it, and create a copy
  jamf-cli pro enrollment-access-groups get 1 -o json | jq '.name = "Copy"' | jamf-cli pro enrollment-access-groups create`,
...
func newEnrollmentAccessGroupsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  # Replace a enrollment-access-group from JSON
  echo '{"name":"Updated"}' | jamf-cli pro enrollment-access-groups update 1

  # Update by name
  jamf-cli pro enrollment-access-groups get --name "Example" -o json | jq '.field = "value"' | jamf-cli pro enrollment-access-groups update --name "Example"

  # Get a enrollment-access-group, modify, and update
  jamf-cli pro enrollment-access-groups get 1 -o json | jq '.name = "New Name"' | jamf-cli pro enrollment-access-groups update 1`,
		Annotations: map[string]string{"jamf:privileges": "Update User-Initiated Enrollment", "jamf:api": "pro", "jamf:gateway-privileges": "user-initiated-enrollment:update"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current enrollment-access-group for --set: %w", err)
					}
				}
...
func newEnrollmentAccessGroupsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
		fromFile   string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Delete a enrollment-access-group (with confirmation)
  jamf-cli pro enrollment-access-groups delete 1

  # Delete by name
  jamf-cli pro enrollment-access-groups delete --name "Example" --yes

  # Delete without confirmation prompt
  jamf-cli pro enrollment-access-groups delete 1 --yes`,
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:privileges": "Update User-Initiated Enrollment", "jamf:api": "pro", "jamf:gateway-privileges": "user-initiated-enrollment:update"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if rid == "" {
							return fmt.Errorf("no enrollment-access-group found matching %q", entry)
						}
				...
				if flagDryRun {
					for _, e := range bulk {
						fmt.Fprintf(os.Stderr, "[dry-run] Would delete enrollment-access-group %q (id: %s)\n", e.label, e.id)
					}
					return nil
				}
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d enrollment-access-groups. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete enrollment-access-group %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete enrollment-access-group %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
					...
					fmt.Fprintf(os.Stderr, "Deleted enrollment-access-group %q (id: %s)\n", e.label, e.id)
					okCount++
				}
				cooldown.Record(ctx.ProfileName)
				return batchDeleteError(cmd, okCount, failCount, firstErr, "enrollment-access-groups deletes")
			}
			...
			if flagName != "" {
				...
				if rid == "" {
					return fmt.Errorf("no enrollment-access-group found with name %q", flagName)
				}
				resolvedID = rid
			...
			if flagDryRun {
				if resolvedByName != "" {
					...
					fmt.Fprintf(os.Stderr, "[dry-run] Would delete enrollment-access-group %s\n", resolvedID)
				}
				return nil
			}
			if !flagYes {
				...
				if resolvedByName != "" {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete enrollment-access-group %q (id: %s). Type 'yes' to confirm: ", resolvedByName, resolvedID)
				} else {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete enrollment-access-group %s. Type 'yes' to confirm: ", resolvedID)
				}
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
...
func newEnrollmentAccessGroupsApplyCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		fromFile     string
		flagYes      bool
		flagDryRun   bool
	...
	cmd := &cobra.Command{
		Use:         "apply",
		Short:       "Create or replace a enrollment-access-group by name",
		Annotations: map[string]string{"jamf:api": "pro", "jamf:gateway-privileges": "user-initiated-enrollment:read,user-initiated-enrollment:update"},
		Long: `Create or replace a enrollment-access-group. Reads JSON or YAML from --from-file or stdin.

The name field in the input is used to check if the resource
already exists. If it does, the resource is replaced (with confirmation).
If not, a new resource is created.`,
		Example: `  # Apply a enrollment-access-group from a JSON file
  jamf-cli pro enrollment-access-groups apply --from-file enrollment-access-group.json

  # Apply a enrollment-access-group from a YAML file
  jamf-cli pro enrollment-access-groups apply --from-file enrollment-access-group.yaml

  # Apply from stdin
  cat enrollment-access-group.json | jamf-cli pro enrollment-access-groups apply

  # Apply without replacement confirmation
  jamf-cli pro enrollment-access-groups apply --from-file enrollment-access-group.json --yes

  # Preview what would happen
  jamf-cli pro enrollment-access-groups apply --from-file enrollment-access-group.json --dry-run`,
  ...
  "siteId": "-1"
			...
			if id == "" {
				// Not found — create
				if flagDryRun {
					fmt.Fprintf(os.Stderr, "[dry-run] Would create enrollment-access-group %q\n", name)
					return nil
				}
				...
				defer resp.Body.Close()
				fmt.Fprintf(os.Stderr, "Created enrollment-access-group %q\n", name)
				return ctx.Output.PrintResponse(resp)
			}

			// Found — replace
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "[dry-run] Would replace enrollment-access-group %q (id: %s)\n", name, id)
				return nil
			}
			if !flagYes {
				if noInput {
					return fmt.Errorf("enrollment-access-group %q already exists (id: %s); use --yes to replace when --no-input is set", name, id)
				}
				fmt.Fprintf(os.Stderr, "enrollment-access-group %q already exists (id: %s) and will be replaced. Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			defer resp.Body.Close()
			fmt.Fprintf(os.Stderr, "Replaced enrollment-access-group %q (id: %s)\n", name, id)
			return ctx.Output.PrintResponse(resp)
		},
	}

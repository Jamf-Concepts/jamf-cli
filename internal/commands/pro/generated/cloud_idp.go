...
// NewCloudIdpCmd creates the cloud-idp command group
func NewCloudIdpCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "cloud-idp",
		Short:       "Manage cloud-idp",
		Long:        `Manage cloud-idp in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newCloudIdpListCmd(ctx))
	cmd.AddCommand(newCloudIdpGetCmd(ctx))
	cmd.AddCommand(newCloudIdpHistoryCmd(ctx))
	cmd.AddCommand(newCloudIdpAddHistoryNoteCmd(ctx))
	cmd.AddCommand(newCloudIdpExportCmd(ctx))
	cmd.AddCommand(newCloudIdpTestGroupCmd(ctx))
	cmd.AddCommand(newCloudIdpTestUserCmd(ctx))
	cmd.AddCommand(newCloudIdpTestUserMembershipCmd(ctx))

	return cmd
}

func newCloudIdpListCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all cloud-idp
  jamf-cli pro cloud-idp list

  # List cloud-idp and extract IDs
  jamf-cli pro cloud-idp list --field id`,
...
func newCloudIdpGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get a cloud-idp by ID
  jamf-cli pro cloud-idp get 1

  # Get a cloud-idp by name
  jamf-cli pro cloud-idp get --name "Example"

  # Get a cloud-idp and output as YAML
  jamf-cli pro cloud-idp get 1 -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "Read LDAP Servers", "jamf:api": "pro", "jamf:gateway-privileges": "ldap-servers:read"},
...
func newCloudIdpHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get history for a cloud-idp by ID
  jamf-cli pro cloud-idp history 1

  # Get history by name
  jamf-cli pro cloud-idp history --name "Example"`,
		Annotations: map[string]string{"jamf:privileges": "Read LDAP Servers", "jamf:api": "pro", "jamf:gateway-privileges": "ldap-servers:read"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newCloudIdpExportCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
	...
	cmd := &cobra.Command{
		...
		Example: `  # Export cloud-idp to CSV
  jamf-cli pro cloud-idp export --out-file cloud-idp.csv`,
		Annotations: map[string]string{"jamf:privileges": "Read LDAP Servers", "jamf:api": "pro", "jamf:gateway-privileges": "ldap-servers:read"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

...
func newSupervisionIdentitiesListCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro supervision-identities list --field id`,
		Annotations: map[string]string{"jamf:privileges": "Read Apple Configurator Enrollment", "jamf:api": "pro"},
...
func newSupervisionIdentitiesCreateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro supervision-identities get 1 -o json | jq '.name = "Copy"' | jamf-cli pro supervision-identities create`,
		Annotations: map[string]string{"jamf:privileges": "Update Apple Configurator Enrollment", "jamf:api": "pro"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newSupervisionIdentitiesUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro supervision-identities update 1 --set field=value

  # Replace a supervision-identity from JSON
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current supervision-identity for --set: %w", err)
					}
				}
...
func newSupervisionIdentitiesDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro supervision-identities delete 1 --yes`,
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:privileges": "Update Apple Configurator Enrollment", "jamf:api": "pro"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

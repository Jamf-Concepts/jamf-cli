			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "diskencryptionconfigurations", "diskencryptionconfigurations", entry, noInputBulk)
							if err != nil {
						...
						if resolvedID == "" {
							return fmt.Errorf("no disk_encryption_configuration found matching %q", entry)
						}
				...
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d diskencryptionconfigurations. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				var firstErr error
				for _, e := range bulk {
					delPath := fmt.Sprintf("/JSSResource/diskencryptionconfigurations/id/%s", url.PathEscape(e.id))
					resp, err := ctx.Client.Do(reqCtx, "DELETE", delPath, nil)
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete disk_encryption_configuration %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode == http.StatusNoContent || resp.StatusCode == http.StatusOK {
						...
						fmt.Fprintf(os.Stderr, "delete disk_encryption_configuration %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
				...
				return batchDeleteError(cmd, okCount, failCount, firstErr, "diskencryptionconfigurations deletes")
			}

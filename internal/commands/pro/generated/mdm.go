...
// NewMdmCmd creates the mdm command group
func NewMdmCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "mdm",
		Short:       "Manage mdm",
		Long:        `Manage mdm in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newMdmListCmd(ctx))
	cmd.AddCommand(newMdmDeployPackageCmd(ctx))
	cmd.AddCommand(newMdmRenewProfileCmd(ctx))
	cmd.AddCommand(newMdmBlankPushCmd(ctx))
	cmd.AddCommand(newMdmCommandsCmd(ctx))

	return cmd
}

func newMdmListCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all mdm
  jamf-cli pro mdm list

  # List mdm and extract IDs
  jamf-cli pro mdm list --field id`,
...
func newMdmDeployPackageCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagVerbose  bool
		flagScaffold bool
	)
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:privileges": "Send Computer Remote Command to Install Package", "jamf:api": "pro", "jamf:gateway-privileges": "device-actions:execute"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newMdmCommandsCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:privileges": "View MDM command information in Jamf Pro API", "jamf:api": "pro", "jamf:gateway": "unserved", "jamf:gateway-basis": "unpublished", "jamf:gateway-detail": "the gateway's Jamf Pro API 11.32.0 declares GET on this path but not POST"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

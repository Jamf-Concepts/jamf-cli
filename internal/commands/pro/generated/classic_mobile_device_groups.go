...
func newClassicMobileDeviceGroupsGetCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-mobile-device-groups get 1 -o yaml`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var path string
			if flagName != "" {
				path = fmt.Sprintf("/JSSResource/mobiledevicegroups/name/%s", url.PathEscape(flagName))
			} else if len(args) > 0 {
...
func newClassicMobileDeviceGroupsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-mobile-device-groups delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "mobiledevicegroups", "mobiledevicegroups", entry, noInputBulk)
							if err != nil {
			...
			if flagName != "" {
				id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "mobiledevicegroups", "mobiledevicegroups", flagName, noInput)
				if err != nil {
					return err
				}
				if id == "" {
...
If not, a new resource is created.`,
  ...
  jamf-cli pro classic-mobile-device-groups apply --from-file mobile_device_group.xml --yes`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			noInput, _ := cmd.Flags().GetBool("no-input")
			id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "mobiledevicegroups", "mobiledevicegroups", name, noInput)
			if err != nil {
				return err
			}

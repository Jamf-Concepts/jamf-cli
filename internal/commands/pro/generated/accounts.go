...
func NewAccountsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "accounts",
		Short:       "Manage accounts",
		Long:        `Manage accounts in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newAccountsListCmd(ctx))
	cmd.AddCommand(newAccountsGetCmd(ctx))
	cmd.AddCommand(newAccountsCreateCmd(ctx))
	cmd.AddCommand(newAccountsUpdateCmd(ctx))
	cmd.AddCommand(newAccountsDeleteCmd(ctx))
	cmd.AddCommand(newAccountsApplyCmd(ctx))

	return cmd
}

func newAccountsListCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
  ...
  jamf-cli pro accounts list

  # List accounts and extract IDs
  jamf-cli pro accounts list --field id`,
...
func newAccountsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
  ...
  jamf-cli pro accounts get 1

  # Get a account by name
  jamf-cli pro accounts get --name "Example"

  # Get a account and output as YAML
  jamf-cli pro accounts get 1 -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "Read Accounts", "jamf:api": "pro", "jamf:gateway-privileges": "accounts:read"},
...
func newAccountsCreateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Show the JSON template for creating a account
  jamf-cli pro accounts create --scaffold

  # Create a account from JSON
...
func newAccountsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  # Replace a account from JSON
  echo '{"name":"Updated"}' | jamf-cli pro accounts update 1
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current account for --set: %w", err)
					}
				}
...
func newAccountsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
		fromFile   string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Delete a account (with confirmation)
  jamf-cli pro accounts delete 1

  # Delete by name
  jamf-cli pro accounts delete --name "Example" --yes

  # Delete without confirmation prompt
  jamf-cli pro accounts delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if rid == "" {
							return fmt.Errorf("no account found matching %q", entry)
						}
				...
				if flagDryRun {
					for _, e := range bulk {
						fmt.Fprintf(os.Stderr, "[dry-run] Would delete account %q (id: %s)\n", e.label, e.id)
					}
					return nil
				}
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d accounts. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete account %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete account %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
					...
					fmt.Fprintf(os.Stderr, "Deleted account %q (id: %s)\n", e.label, e.id)
					okCount++
				}
				cooldown.Record(ctx.ProfileName)
				return batchDeleteError(cmd, okCount, failCount, firstErr, "accounts deletes")
			}
			...
			if flagName != "" {
				...
				if rid == "" {
					return fmt.Errorf("no account found with username %q", flagName)
				}
				resolvedID = rid
			...
			if flagDryRun {
				if resolvedByName != "" {
					...
					fmt.Fprintf(os.Stderr, "[dry-run] Would delete account %s\n", resolvedID)
				}
				return nil
			}
			if !flagYes {
				...
				if resolvedByName != "" {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete account %q (id: %s). Type 'yes' to confirm: ", resolvedByName, resolvedID)
				} else {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete account %s. Type 'yes' to confirm: ", resolvedID)
				}
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
...
func newAccountsApplyCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		fromFile     string
		flagYes      bool
		flagDryRun   bool
	...
	cmd := &cobra.Command{
		Use:         "apply",
		Short:       "Create or replace a account by name",
		Annotations: map[string]string{"jamf:api": "pro", "jamf:gateway-privileges": "accounts:create,accounts:read,accounts:update"},
		Long: `Create or replace a account. Reads JSON or YAML from --from-file or stdin.

The username field in the input is used to check if the resource
already exists. If it does, the resource is replaced (with confirmation).
If not, a new resource is created.`,
		Example: `  # Apply a account from a JSON file
  jamf-cli pro accounts apply --from-file account.json

  # Apply a account from a YAML file
  jamf-cli pro accounts apply --from-file account.yaml

  # Apply from stdin
  cat account.json | jamf-cli pro accounts apply

  # Apply without replacement confirmation
  jamf-cli pro accounts apply --from-file account.json --yes

  # Preview what would happen
  jamf-cli pro accounts apply --from-file account.json --dry-run`,
  ...
  "username": "testusername"
			...
			if id == "" {
				// Not found — create
				if flagDryRun {
					fmt.Fprintf(os.Stderr, "[dry-run] Would create account %q\n", name)
					return nil
				}
				...
				defer resp.Body.Close()
				fmt.Fprintf(os.Stderr, "Created account %q\n", name)
				return ctx.Output.PrintResponse(resp)
			}

			// Found — replace
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "[dry-run] Would replace account %q (id: %s)\n", name, id)
				return nil
			}
			if !flagYes {
				if noInput {
					return fmt.Errorf("account %q already exists (id: %s); use --yes to replace when --no-input is set", name, id)
				}
				fmt.Fprintf(os.Stderr, "account %q already exists (id: %s) and will be replaced. Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			defer resp.Body.Close()
			fmt.Fprintf(os.Stderr, "Replaced account %q (id: %s)\n", name, id)
			return ctx.Output.PrintResponse(resp)
		},
	}

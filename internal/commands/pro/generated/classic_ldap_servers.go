...
func newClassicLdapServersDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-ldap-servers delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "ldapservers", "ldapservers", entry, noInputBulk)
							if err != nil {
			...
			if flagName != "" {
				id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "ldapservers", "ldapservers", flagName, noInput)
				if err != nil {
					return err
				}
				if id == "" {
...
If not, a new resource is created.`,
  ...
  jamf-cli pro classic-ldap-servers apply --from-file ldap_server.xml --yes`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			noInput, _ := cmd.Flags().GetBool("no-input")
			id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "ldapservers", "ldapservers", name, noInput)
			if err != nil {
				return err
			}

...
func NewVppSubscriptionsCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd.AddCommand(newVppSubscriptionsGetByNameCmd(ctx))
	cmd.AddCommand(newVppSubscriptionsApplyCmd(ctx))
	cmd.AddCommand(newVppSubscriptionsDeleteByNameCmd(ctx))
...
func newVppSubscriptionsGetCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli vpp-subscriptions get-by-name "Example"
...
func newVppSubscriptionsGetByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	return &cobra.Command{
		Use:   "get-by-name <name>",
		Short: "Get a vpp-subscription by name",
		Example: `  # Get a vpp-subscription by name
  jamf-cli vpp-subscriptions get-by-name "Example"

  # Get by name and output as YAML
  jamf-cli vpp-subscriptions get-by-name "Example" -o yaml`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			id, err := resolveNameToID(reqCtx, ctx.Client, "/v1/volume-purchasing-subscriptions", "name", "id", args[0])
			if err != nil {
				return err
			}
			path := strings.Replace("/v1/volume-purchasing-subscriptions/{id}", "{id}", url.PathEscape(id), 1)
			resp, err := ctx.Client.Do(reqCtx, "GET", path, nil)
			if err != nil {
				return err
			}
...
func newVppSubscriptionsDeleteByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
	)
	...
	cmd := &cobra.Command{
		...
		Short: "Delete a vpp-subscription by name",
		Example: `  # Delete a vpp-subscription by name (with confirmation)
  jamf-cli vpp-subscriptions delete-by-name "Example"

  # Delete without confirmation prompt
  jamf-cli vpp-subscriptions delete-by-name "Example" --yes`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			name := args[0]
			...
			if id == "" {
				return fmt.Errorf("no vpp-subscription found with name %q", name)
			}
			...
			if !flagYes {
				...
				fmt.Fprintf(os.Stderr, "This will delete vpp-subscription %q (id: %s). Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")

...
func newClassicComputerConfigsGetCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-computer-configs get 1 -o yaml`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var path string
			if flagName != "" {
				path = fmt.Sprintf("/JSSResource/computerconfigurations/name/%s", url.PathEscape(flagName))
			} else if len(args) > 0 {
...
func newClassicComputerConfigsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-computer-configs delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "computerconfigurations", "computerconfigurations", entry, noInputBulk)
							if err != nil {
						...
						if resolvedID == "" {
							return fmt.Errorf("no computer_configuration found matching %q", entry)
						}
				...
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d computerconfigurations. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				var firstErr error
				for _, e := range bulk {
					delPath := fmt.Sprintf("/JSSResource/computerconfigurations/id/%s", url.PathEscape(e.id))
					resp, err := ctx.Client.Do(reqCtx, "DELETE", delPath, nil)
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete computer_configuration %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode == http.StatusNoContent || resp.StatusCode == http.StatusOK {
						...
						fmt.Fprintf(os.Stderr, "delete computer_configuration %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
				...
				return batchDeleteError(cmd, okCount, failCount, firstErr, "computerconfigurations deletes")
			}
			...
			if flagName != "" {
				id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "computerconfigurations", "computerconfigurations", flagName, noInput)
				if err != nil {
					return err
				}
				if id == "" {
			...
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "[dry-run] Would delete computer_configuration %s\n", resolvedID)
				return nil
			}
			if !flagYes {
				if noInput {
				...
				fmt.Fprintf(os.Stderr, "This will delete computer_configuration %s. Type 'yes' to confirm: ", resolvedID)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			path := fmt.Sprintf("/JSSResource/computerconfigurations/id/%s", url.PathEscape(resolvedID))

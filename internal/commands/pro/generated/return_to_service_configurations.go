...
func newReturnToServiceConfigurationsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro return-to-service-configurations update 1 --set field=value

  # Replace a return-to-service-configuration from JSON
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current return-to-service-configuration for --set: %w", err)
					}
				}

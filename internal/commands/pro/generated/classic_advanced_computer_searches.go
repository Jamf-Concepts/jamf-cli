...
func newClassicAdvancedComputerSearchesDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-advanced-computer-searches delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "advancedcomputersearches", "advancedcomputersearches", entry, noInputBulk)
							if err != nil {
			...
			if flagName != "" {
				id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "advancedcomputersearches", "advancedcomputersearches", flagName, noInput)
				if err != nil {
					return err
				}
				if id == "" {

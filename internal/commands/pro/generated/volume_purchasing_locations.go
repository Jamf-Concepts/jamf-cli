...
// NewVolumePurchasingLocationsCmd creates the volume-purchasing-locations command group
func NewVolumePurchasingLocationsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "volume-purchasing-locations",
		Short:       "Manage volume-purchasing-locations",
		Long:        `Manage volume-purchasing-locations in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newVolumePurchasingLocationsListCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsGetCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsCreateCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsDeleteCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsHistoryCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsAddHistoryNoteCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsPatchCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsContentCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsReclaimCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsRevokeLicensesCmd(ctx))
	cmd.AddCommand(newVolumePurchasingLocationsApplyCmd(ctx))

	return cmd
}

func newVolumePurchasingLocationsListCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all volume-purchasing-locations
  jamf-cli pro volume-purchasing-locations list

  # List volume-purchasing-locations and extract IDs
  jamf-cli pro volume-purchasing-locations list --field id`,
...
func newVolumePurchasingLocationsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get a volume-purchasing-location by ID
  jamf-cli pro volume-purchasing-locations get 1

  # Get a volume-purchasing-location by name
  jamf-cli pro volume-purchasing-locations get --name "Example"

  # Get a volume-purchasing-location and output as YAML
  jamf-cli pro volume-purchasing-locations get 1 -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "Read Volume Purchasing Locations", "jamf:api": "pro", "jamf:gateway-privileges": "volume-purchasing-locations:read"},
...
func newVolumePurchasingLocationsCreateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Show the JSON template for creating a volume-purchasing-location
  jamf-cli pro volume-purchasing-locations create --scaffold

  # Create a volume-purchasing-location from JSON
  echo '{"name":"Example"}' | jamf-cli pro volume-purchasing-locations create

  # Get a volume-purchasing-location, modify it, and create a copy
  jamf-cli pro volume-purchasing-locations get 1 -o json | jq '.name = "Copy"' | jamf-cli pro volume-purchasing-locations create`,
...
func newVolumePurchasingLocationsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
		fromFile   string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Delete a volume-purchasing-location (with confirmation)
  jamf-cli pro volume-purchasing-locations delete 1

  # Delete by name
  jamf-cli pro volume-purchasing-locations delete --name "Example" --yes

  # Delete without confirmation prompt
  jamf-cli pro volume-purchasing-locations delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						var rid string
						if rid == "" {
							id, err := resolveNameToIDForApply(reqCtx, ctx.Client, "/v1/volume-purchasing-locations", "name", "id", entry, noInputBulk)
							if err != nil {
						...
						if rid == "" {
							return fmt.Errorf("no volume-purchasing-location found matching %q", entry)
						}
				...
				if flagDryRun {
					for _, e := range bulk {
						fmt.Fprintf(os.Stderr, "[dry-run] Would delete volume-purchasing-location %q (id: %s)\n", e.label, e.id)
					}
					return nil
				}
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d volume-purchasing-locations. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				var firstErr error
				for _, e := range bulk {
					delPath := strings.Replace("/v1/volume-purchasing-locations/{id}", "{id}", url.PathEscape(e.id), 1)
					resp, err := ctx.Client.Do(reqCtx, "DELETE", delPath, nil)
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete volume-purchasing-location %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete volume-purchasing-location %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
					...
					fmt.Fprintf(os.Stderr, "Deleted volume-purchasing-location %q (id: %s)\n", e.label, e.id)
					okCount++
				}
				cooldown.Record(ctx.ProfileName)
				return batchDeleteError(cmd, okCount, failCount, firstErr, "volume-purchasing-locations deletes")
			}
			...
			if flagName != "" {
				...
				if rid == "" {
					return fmt.Errorf("no volume-purchasing-location found with name %q", flagName)
				}
				resolvedID = rid
			...
			if flagDryRun {
				if resolvedByName != "" {
					...
					fmt.Fprintf(os.Stderr, "[dry-run] Would delete volume-purchasing-location %s\n", resolvedID)
				}
				return nil
			}
			if !flagYes {
				...
				if resolvedByName != "" {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete volume-purchasing-location %q (id: %s). Type 'yes' to confirm: ", resolvedByName, resolvedID)
				} else {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete volume-purchasing-location %s. Type 'yes' to confirm: ", resolvedID)
				}
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
...
func newVolumePurchasingLocationsHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get history for a volume-purchasing-location by ID
  jamf-cli pro volume-purchasing-locations history 1

  # Get history by name
  jamf-cli pro volume-purchasing-locations history --name "Example"`,
		Annotations: map[string]string{"jamf:privileges": "Read Volume Purchasing Locations", "jamf:api": "pro", "jamf:gateway-privileges": "volume-purchasing-locations:read"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newVolumePurchasingLocationsPatchCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
		flagSet      []string
		fromFile     string
  ...
  jamf-cli pro volume-purchasing-locations patch 1 --set field1=value1 --set field2=value2

  # Update by name
  jamf-cli pro volume-purchasing-locations patch --name "Example" --set general.managed=true

  # Patch from a file
  jamf-cli pro volume-purchasing-locations patch 1 --from-file changes.json`,
		Annotations: map[string]string{"jamf:privileges": "Update Volume Purchasing Locations", "jamf:api": "pro", "jamf:gateway-privileges": "volume-purchasing-locations:update"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newVolumePurchasingLocationsContentCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flagAll && flagPage == 0 {
				...
				for {
					// Build page-specific query
					pagePath := "/v1/volume-purchasing-locations/{id}/content"
					pagePath = strings.Replace(pagePath, "{id}", url.PathEscape(resolvedID), 1)
					var pageQuery []string
					// Carry forward non-pagination query params
					for _, qp := range queryParts {
...
func newVolumePurchasingLocationsReclaimCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
...
func newVolumePurchasingLocationsRevokeLicensesCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
...
func newVolumePurchasingLocationsApplyCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "apply",
		Short:       "Create or replace a volume-purchasing-location by name",
		Annotations: map[string]string{"jamf:api": "pro", "jamf:gateway-privileges": "volume-purchasing-locations:create,volume-purchasing-locations:read"},
		Long: `Create or replace a volume-purchasing-location. Reads JSON or YAML from --from-file or stdin.

The name field in the input is used to check if the resource
already exists. If it does, the resource is replaced (with confirmation).
If not, a new resource is created.`,
		Example: `  # Apply a volume-purchasing-location from a JSON file
  jamf-cli pro volume-purchasing-locations apply --from-file volume-purchasing-location.json

  # Apply a volume-purchasing-location from a YAML file
  jamf-cli pro volume-purchasing-locations apply --from-file volume-purchasing-location.yaml

  # Apply from stdin
  cat volume-purchasing-location.json | jamf-cli pro volume-purchasing-locations apply

  # Apply without replacement confirmation
  jamf-cli pro volume-purchasing-locations apply --from-file volume-purchasing-location.json --yes

  # Preview what would happen
  jamf-cli pro volume-purchasing-locations apply --from-file volume-purchasing-location.json --dry-run`,
  ...
  "siteId": "1"
			...
			if id == "" {
				// Not found — create
				if flagDryRun {
					fmt.Fprintf(os.Stderr, "[dry-run] Would create volume-purchasing-location %q\n", name)
					return nil
				}
				...
				defer resp.Body.Close()
				fmt.Fprintf(os.Stderr, "Created volume-purchasing-location %q\n", name)
				return ctx.Output.PrintResponse(resp)
			}

			// Found — replace
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "[dry-run] Would replace volume-purchasing-location %q (id: %s)\n", name, id)
				return nil
			}
			if !flagYes {
				if noInput {
					return fmt.Errorf("volume-purchasing-location %q already exists (id: %s); use --yes to replace when --no-input is set", name, id)
				}
				fmt.Fprintf(os.Stderr, "volume-purchasing-location %q already exists (id: %s) and will be replaced. Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			defer resp.Body.Close()
			fmt.Fprintf(os.Stderr, "Replaced volume-purchasing-location %q (id: %s)\n", name, id)
			return ctx.Output.PrintResponse(resp)
		},
	}

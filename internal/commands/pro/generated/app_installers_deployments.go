...
// NewAppInstallersDeploymentsCmd creates the app-installers-deployments command group
func NewAppInstallersDeploymentsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "app-installers-deployments",
		Short:       "Manage app-installers-deployments",
		Long:        `Manage app-installers-deployments in Jamf Pro.`,
...
func newAppInstallersDeploymentsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get a app-installers-deployment by ID
  jamf-cli pro app-installers-deployments get 1

  # Get a app-installers-deployment by name
  jamf-cli pro app-installers-deployments get --name "Example"

  # Get a app-installers-deployment and output as YAML
  jamf-cli pro app-installers-deployments get 1 -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "Read Mac Applications", "jamf:api": "pro", "jamf:gateway-privileges": "applications:read"},
...
func newAppInstallersDeploymentsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  # Replace a app-installers-deployment from JSON
  echo '{"name":"Updated"}' | jamf-cli pro app-installers-deployments update 1

  # Update by name
  jamf-cli pro app-installers-deployments get --name "Example" -o json | jq '.field = "value"' | jamf-cli pro app-installers-deployments update --name "Example"

  # Get a app-installers-deployment, modify, and update
  jamf-cli pro app-installers-deployments get 1 -o json | jq '.name = "New Name"' | jamf-cli pro app-installers-deployments update 1`,
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current app-installers-deployment for --set: %w", err)
					}
				}
				...
				setDoc, serr := buildMergePatchFromSet(flagSet, map[string]string{"appTitleId": "string", "categoryId": "string", "deploymentType": "string", "enabled": "boolean", "installPredefinedConfigProfiles": "boolean", "name": "string", "notificationSettings": "object", "notificationSettings.completeMessage": "string", "notificationSettings.deadline": "integer", "notificationSettings.deadlineMessage": "string", "notificationSettings.notificationInterval": "integer", "notificationSettings.notificationMessage": "string", "notificationSettings.quitDelay": "integer", "notificationSettings.relaunch": "boolean", "notificationSettings.suppress": "boolean", "selfServiceSettings": "object", "selfServiceSettings.categories": "array", "selfServiceSettings.description": "string", "selfServiceSettings.forceViewDescription": "boolean", "selfServiceSettings.includeInComplianceCategory": "boolean", "selfServiceSettings.includeInFeaturedCategory": "boolean", "siteId": "string", "smartGroupId": "string", "triggerAdminNotifications": "boolean", "updateBehavior": "string"})
				if serr != nil {
					return serr
				}
...
func newAppInstallersDeploymentsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
		fromFile   string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Delete a app-installers-deployment (with confirmation)
  jamf-cli pro app-installers-deployments delete 1

  # Delete by name
  jamf-cli pro app-installers-deployments delete --name "Example" --yes

  # Delete without confirmation prompt
  jamf-cli pro app-installers-deployments delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if rid == "" {
							return fmt.Errorf("no app-installers-deployment found matching %q", entry)
						}
				...
				if flagDryRun {
					for _, e := range bulk {
						fmt.Fprintf(os.Stderr, "[dry-run] Would delete app-installers-deployment %q (id: %s)\n", e.label, e.id)
					}
					return nil
				}
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d app-installers-deployments. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete app-installers-deployment %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete app-installers-deployment %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
					...
					fmt.Fprintf(os.Stderr, "Deleted app-installers-deployment %q (id: %s)\n", e.label, e.id)
					okCount++
				}
				cooldown.Record(ctx.ProfileName)
				return batchDeleteError(cmd, okCount, failCount, firstErr, "app-installers-deployments deletes")
			}
			...
			if flagName != "" {
				...
				if rid == "" {
					return fmt.Errorf("no app-installers-deployment found with name %q", flagName)
				}
				resolvedID = rid
			...
			if flagDryRun {
				if resolvedByName != "" {
					...
					fmt.Fprintf(os.Stderr, "[dry-run] Would delete app-installers-deployment %s\n", resolvedID)
				}
				return nil
			}
			if !flagYes {
				...
				if resolvedByName != "" {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete app-installers-deployment %q (id: %s). Type 'yes' to confirm: ", resolvedByName, resolvedID)
				} else {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete app-installers-deployment %s. Type 'yes' to confirm: ", resolvedID)
				}
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
...
func newAppInstallersDeploymentsHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get history for a app-installers-deployment by ID
  jamf-cli pro app-installers-deployments history 1

  # Get history by name
  jamf-cli pro app-installers-deployments history --name "Example"`,
		Annotations: map[string]string{"jamf:privileges": "Read Mac Applications", "jamf:api": "pro", "jamf:gateway-privileges": "applications:read"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newAppInstallersDeploymentsExportCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
	...
	cmd := &cobra.Command{
		...
		Example: `  # Export app-installers-deployments to CSV
  jamf-cli pro app-installers-deployments export --out-file app-installers-deployments.csv`,
		Annotations: map[string]string{"jamf:privileges": "Read Mac Applications", "jamf:api": "pro", "jamf:gateway-privileges": "applications:read"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newAppInstallersDeploymentsInstallationRetryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagAll  bool
		flagYes  bool
		flagName string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// the action across every app-installers-deployment in a single server-side
			// call (no per-resource {id} needed).
			if flagAll {
				if len(args) > 0 || flagName != "" {
					return fmt.Errorf("--all applies to every app-installers-deployment; do not combine it with an <id> or --name")
				}
				// Tenant-wide blast radius: installation-retry across every
				// app-installers-deployment in one call. Gate behind an explicit
				// confirmation, matching this codebase's convention for
				// wide-reaching mutations.
				if !flagYes {
					noInput, _ := cmd.Flags().GetBool("no-input")
					if noInput {
						return fmt.Errorf("--all installation-retry applies to every app-installers-deployment; pass --yes to confirm when --no-input is set")
					}
					fmt.Fprintf(os.Stderr, "⚠️  --all will installation-retry across every app-installers-deployment in this tenant. Type 'yes' to confirm: ")
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
						return fmt.Errorf("aborted")
	...
	cmd.Flags().BoolVar(&flagAll, "all", false, "Apply to every app-installers-deployment in one call (collection-level installation-retry endpoint)")
...
func newAppInstallersDeploymentsInstallationRetryByComputerIdCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()

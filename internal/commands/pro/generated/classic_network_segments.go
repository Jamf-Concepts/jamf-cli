...
func newClassicNetworkSegmentsGetCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-network-segments get 1 -o yaml`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var path string
			if flagName != "" {
				path = fmt.Sprintf("/JSSResource/networksegments/name/%s", url.PathEscape(flagName))
			} else if len(args) > 0 {
...
func newClassicNetworkSegmentsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-network-segments delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "networksegments", "networksegments", entry, noInputBulk)
							if err != nil {
			...
			if flagName != "" {
				id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "networksegments", "networksegments", flagName, noInput)
				if err != nil {
					return err
				}
				if id == "" {

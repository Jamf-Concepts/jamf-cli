...
// NewAppInstallersGlobalSettingsCmd creates the app-installers global-settings command group
func NewAppInstallersGlobalSettingsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "global-settings",
		Short:       "Manage app-installers global-settings",
		Long:        `Manage app-installers global-settings in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newAppInstallersGlobalSettingsGetCmd(ctx))
	cmd.AddCommand(newAppInstallersGlobalSettingsUpdateCmd(ctx))
	cmd.AddCommand(newAppInstallersGlobalSettingsHistoryCmd(ctx))
	cmd.AddCommand(newAppInstallersGlobalSettingsAddHistoryNoteCmd(ctx))
	cmd.AddCommand(newAppInstallersGlobalSettingsDeploymentControlsCmd(ctx))

	return cmd
}

func newAppInstallersGlobalSettingsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get app-installers global-settings
  jamf-cli pro app-installers global-settings get

  # Get app-installers global-settings and output as YAML
  jamf-cli pro app-installers global-settings get -o yaml`,
...
func newAppInstallersGlobalSettingsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  # Replace app-installers global-settings from a full JSON document
  jamf-cli pro app-installers global-settings get -o json | jq '.field = "value"' | jamf-cli pro app-installers global-settings update

  # Update from a file
  jamf-cli pro app-installers global-settings update --from-file app-installers-global-settings.json`,
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current global-settings for --set: %w", err)
					}
				}
...
func newAppInstallersGlobalSettingsHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get history for the global-settings
...
func newAppInstallersGlobalSettingsDeploymentControlsCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()

...
func newAdvancedUserContentSearchesUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro advanced-user-content-searches update 1 --set field=value

  # Replace a advanced-user-content-searche from JSON
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current advanced-user-content-searche for --set: %w", err)
					}
				}
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						var rid string
						if rid == "" {
							id, err := resolveNameToIDForApply(reqCtx, ctx.Client, "/v1/advanced-user-content-searches", "name", "id", entry, noInputBulk)
							if err != nil {
						...
						if rid == "" {
							return fmt.Errorf("no advanced-user-content-searche found matching %q", entry)
						}
				...
				var firstErr error
				for _, e := range bulk {
					delPath := strings.Replace("/v1/advanced-user-content-searches/{id}", "{id}", url.PathEscape(e.id), 1)
					resp, err := ctx.Client.Do(reqCtx, "DELETE", delPath, nil)
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete advanced-user-content-searche %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete advanced-user-content-searche %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
					...
					fmt.Fprintf(os.Stderr, "Deleted advanced-user-content-searche %q (id: %s)\n", e.label, e.id)
					okCount++
				}
				cooldown.Record(ctx.ProfileName)
				return batchDeleteError(cmd, okCount, failCount, firstErr, "advanced-user-content-searches deletes")
			}

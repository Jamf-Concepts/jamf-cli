...
// NewDeviceEnrollmentsCmd creates the device-enrollments command group
func NewDeviceEnrollmentsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "device-enrollments",
		Short:       "Manage device-enrollments",
		Long:        `Manage device-enrollments in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newDeviceEnrollmentsListCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsGetCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsCreateCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsUpdateCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsDeleteCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsHistoryCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsAddHistoryNoteCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsSyncsCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsPublicKeyCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsDevicesCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsDisownCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsSyncsByIdCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsLatestCmd(ctx))
	cmd.AddCommand(newDeviceEnrollmentsApplyCmd(ctx))

	return cmd
}

func newDeviceEnrollmentsListCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all device-enrollments
  jamf-cli pro device-enrollments list

  # List device-enrollments and extract IDs
  jamf-cli pro device-enrollments list --field id`,
...
func newDeviceEnrollmentsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get a device-enrollment by ID
  jamf-cli pro device-enrollments get 1

  # Get a device-enrollment by name
  jamf-cli pro device-enrollments get --name "Example"

  # Get a device-enrollment and output as YAML
  jamf-cli pro device-enrollments get 1 -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "Read Device Enrollment Program Instances", "jamf:api": "pro", "jamf:gateway-privileges": "device-enrollment-program-instances:read"},
...
func newDeviceEnrollmentsCreateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Show the JSON template for creating a device-enrollment
  jamf-cli pro device-enrollments create --scaffold

  # Create a device-enrollment from JSON
  echo '{"name":"Example"}' | jamf-cli pro device-enrollments create

  # Get a device-enrollment, modify it, and create a copy
  jamf-cli pro device-enrollments get 1 -o json | jq '.name = "Copy"' | jamf-cli pro device-enrollments create`,
...
func newDeviceEnrollmentsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Replace a device-enrollment from JSON
  echo '{"name":"Updated"}' | jamf-cli pro device-enrollments update 1

  # Update by name
  jamf-cli pro device-enrollments get --name "Example" -o json | jq '.field = "value"' | jamf-cli pro device-enrollments update --name "Example"

  # Get a device-enrollment, modify, and update
  jamf-cli pro device-enrollments get 1 -o json | jq '.name = "New Name"' | jamf-cli pro device-enrollments update 1`,
...
func newDeviceEnrollmentsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
		fromFile   string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Delete a device-enrollment (with confirmation)
  jamf-cli pro device-enrollments delete 1

  # Delete by name
  jamf-cli pro device-enrollments delete --name "Example" --yes

  # Delete without confirmation prompt
  jamf-cli pro device-enrollments delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if rid == "" {
							return fmt.Errorf("no device-enrollment found matching %q", entry)
						}
				...
				if flagDryRun {
					for _, e := range bulk {
						fmt.Fprintf(os.Stderr, "[dry-run] Would delete device-enrollment %q (id: %s)\n", e.label, e.id)
					}
					return nil
				}
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d device-enrollments. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete device-enrollment %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete device-enrollment %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
					...
					fmt.Fprintf(os.Stderr, "Deleted device-enrollment %q (id: %s)\n", e.label, e.id)
					okCount++
				}
				cooldown.Record(ctx.ProfileName)
				return batchDeleteError(cmd, okCount, failCount, firstErr, "device-enrollments deletes")
			}
			...
			if flagName != "" {
				...
				if rid == "" {
					return fmt.Errorf("no device-enrollment found with name %q", flagName)
				}
				resolvedID = rid
			...
			if flagDryRun {
				if resolvedByName != "" {
					...
					fmt.Fprintf(os.Stderr, "[dry-run] Would delete device-enrollment %s\n", resolvedID)
				}
				return nil
			}
			if !flagYes {
				...
				if resolvedByName != "" {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete device-enrollment %q (id: %s). Type 'yes' to confirm: ", resolvedByName, resolvedID)
				} else {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete device-enrollment %s. Type 'yes' to confirm: ", resolvedID)
				}
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
...
func newDeviceEnrollmentsHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get history for a device-enrollment by ID
  jamf-cli pro device-enrollments history 1

  # Get history by name
  jamf-cli pro device-enrollments history --name "Example"`,
		Annotations: map[string]string{"jamf:privileges": "Read Device Enrollment Program Instances", "jamf:api": "pro", "jamf:gateway-privileges": "device-enrollment-program-instances:read"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newDeviceEnrollmentsSyncsCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
...
func newDeviceEnrollmentsPublicKeyCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagSaveTo string
	)
  ...
  jamf-cli pro device-enrollments public-key -O output.bin

  # Pipe to stdout
  jamf-cli pro device-enrollments public-key > output.bin`,
		Annotations: map[string]string{"jamf:privileges": "Read Device Enrollment Program Instances", "jamf:api": "pro", "jamf:gateway-privileges": "device-enrollment-program-instances:read"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newDeviceEnrollmentsDevicesCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
...
func newDeviceEnrollmentsSyncsByIdCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
...
func newDeviceEnrollmentsLatestCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
...
func newDeviceEnrollmentsApplyCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		fromFile      string
		flagYes       bool
		flagDryRun    bool
	...
	cmd := &cobra.Command{
		Use:         "apply",
		Short:       "Create or replace a device-enrollment by name",
		...
		Long: `Create or replace a device-enrollment. Reads JSON or YAML from --from-file or stdin.

The name field in the input is used to check if the resource
already exists. If it does, the resource is replaced (with confirmation).
If not, a new resource is created.`,
		Example: `  # Apply a device-enrollment from a JSON file
  jamf-cli pro device-enrollments apply --from-file device-enrollment.json

  # Apply a device-enrollment from a YAML file
  jamf-cli pro device-enrollments apply --from-file device-enrollment.yaml

  # Apply from stdin
  cat device-enrollment.json | jamf-cli pro device-enrollments apply

  # Apply without replacement confirmation
  jamf-cli pro device-enrollments apply --from-file device-enrollment.json --yes

  # Preview what would happen
  jamf-cli pro device-enrollments apply --from-file device-enrollment.json --dry-run`,
  ...
  "tokenFileName": "Acme MDM Token"
			...
			if id == "" {
				// Not found — create
				if flagDryRun {
					fmt.Fprintf(os.Stderr, "[dry-run] Would create device-enrollment %q\n", name)
					return nil
				}
				if len(tokenBody) == 0 {
					return fmt.Errorf("cannot create device-enrollment %q: --token-file is required", name)
				}
			...
			// Found — replace
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "[dry-run] Would replace device-enrollment %q (id: %s)\n", name, id)
				return nil
			}
			if !flagYes {
				if noInput {
					return fmt.Errorf("device-enrollment %q already exists (id: %s); use --yes to replace when --no-input is set", name, id)
				}
				fmt.Fprintf(os.Stderr, "device-enrollment %q already exists (id: %s) and will be replaced. Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			if len(data) == 0 && tokenResp != nil {
				...
				defer tokenResp.Body.Close()
				fmt.Fprintf(os.Stderr, "Replaced device-enrollment token %q (id: %s)\n", name, id)
				return ctx.Output.PrintResponse(tokenResp)
			}
			if tokenResp != nil {
				_ = tokenResp.Body.Close()
			...
			defer resp.Body.Close()
			fmt.Fprintf(os.Stderr, "Replaced device-enrollment %q (id: %s)\n", name, id)
			return ctx.Output.PrintResponse(resp)
		},
	}
	...
	cmd.Flags().StringVar(&flagBodyName, "name", "", "Name for the device-enrollment (sets the body's name field)")

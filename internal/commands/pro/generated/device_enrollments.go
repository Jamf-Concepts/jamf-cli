...
func NewDeviceEnrollmentsCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd.AddCommand(newDeviceEnrollmentsUploadTokenCmd(ctx))
	...
	cmd.AddCommand(newDeviceEnrollmentsUploadTokenByIdCmd(ctx))
...
func newDeviceEnrollmentsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro device-enrollments update 1 --set field=value

  # Replace a device-enrollment from JSON
  ...
  "supervisionIdentityId": "1"
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current device-enrollment for --set: %w", err)
					}
				}
				(&fieldFilter{fields: map[string]*fieldFilter{"name": nil, "siteId": nil, "supervisionIdentityId": nil}}).apply(current)
				setDoc, serr := buildMergePatchFromSet(flagSet, map[string]string{"name": "string", "siteId": "string", "supervisionIdentityId": "string"})
				if serr != nil {
					return serr
				}
	...
	_ = cmd.RegisterFlagCompletionFunc("set", func(_ *cobra.Command, _ []string, _ string) ([]string, cobra.ShellCompDirective) {
		return []string{
			"name=", "siteId=", "supervisionIdentityId=",
...
func newDeviceEnrollmentsUploadTokenCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
	)
...
func newDeviceEnrollmentsUploadTokenByIdCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
		flagName     string
	)

...
// NewJamfConnectConfigProfilesCmd creates the jamf-connect-config-profiles command group
func NewJamfConnectConfigProfilesCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "jamf-connect-config-profiles",
		Short:       "Manage jamf-connect-config-profiles",
		Long:        `Manage jamf-connect-config-profiles in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newJamfConnectConfigProfilesListCmd(ctx))
	cmd.AddCommand(newJamfConnectConfigProfilesUpdateCmd(ctx))

	return cmd
}

func newJamfConnectConfigProfilesListCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all jamf-connect-config-profiles
  jamf-cli pro jamf-connect-config-profiles list

  # List jamf-connect-config-profiles and extract IDs
  jamf-cli pro jamf-connect-config-profiles list --field id`,
...
func newJamfConnectConfigProfilesUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Replace a jamf-connect-config-profile from JSON
  echo '{"name":"Updated"}' | jamf-cli pro jamf-connect-config-profiles update 1`,

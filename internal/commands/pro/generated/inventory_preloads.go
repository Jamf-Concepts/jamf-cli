...
func newInventoryPreloadsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro inventory-preloads update 1 --set field=value

  # Replace a inventory-preload from JSON
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current inventory-preload for --set: %w", err)
					}
				}
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if rid == "" {
							return fmt.Errorf("no inventory-preload found matching %q", entry)
						}
				...
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d inventory-preloads. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete inventory-preload %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete inventory-preload %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
				...
				return batchDeleteError(cmd, okCount, failCount, firstErr, "inventory-preloads deletes")
			}

...
// NewSchedulerJobsCmd creates the scheduler-jobs command group
func NewSchedulerJobsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "scheduler-jobs",
		Short:       "Manage scheduler-jobs",
		Long:        `Manage scheduler-jobs in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newSchedulerJobsListCmd(ctx))
	cmd.AddCommand(newSchedulerJobsTriggersCmd(ctx))

	return cmd
}

func newSchedulerJobsListCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all scheduler-jobs
  jamf-cli pro scheduler-jobs list

  # List scheduler-jobs and extract IDs
  jamf-cli pro scheduler-jobs list --field id`,
		Annotations: map[string]string{"jamf:api": "pro"},
...
func newSchedulerJobsTriggersCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd.Flags().StringVar(&flagName, "name", "", "Look up scheduler-job by name")

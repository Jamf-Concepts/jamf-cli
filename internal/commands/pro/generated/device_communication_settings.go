...
func newDeviceCommunicationSettingsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro device-communication-settings update --from-file device-communication-settings.json`,
		Annotations: map[string]string{"jamf:privileges": "Update Automatically Renew MDM Profile Settings", "jamf:api": "pro", "jamf:gateway-privileges": "mdm-profile-renewal-settings:update"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						var rid string
						if rid == "" {
							id, err := resolveNameToIDForApply(reqCtx, ctx.Client, "/v1/mobile-device-groups/smart-groups", "groupName", "groupId", entry, noInputBulk)
							if err != nil {
				...
				for _, e := range bulk {
					delPath := strings.Replace("/v1/mobile-device-groups/smart-groups/{id}", "{id}", url.PathEscape(e.id), 1)

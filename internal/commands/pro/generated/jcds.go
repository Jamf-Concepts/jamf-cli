...
func newJcdsListCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro jcds list --field id`,
		Annotations: map[string]string{"jamf:privileges": "Read Jamf Cloud Distribution Service Files", "jamf:api": "pro"},
...
func newJcdsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro jcds delete 1 --yes`,
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:privileges": "Delete Jamf Cloud Distribution Service Files", "jamf:api": "pro"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						var rid string
						if rid == "" {
							id, err := resolveNameToIDForApply(reqCtx, ctx.Client, "/v1/jcds/files", "name", "fileName", entry, noInputBulk)
							if err != nil {
						...
						if rid == "" {
							return fmt.Errorf("no jcd found matching %q", entry)
						}
				...
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d jcds. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete jcd %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete jcd %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
				...
				return batchDeleteError(cmd, okCount, failCount, firstErr, "jcds deletes")
			}
...
func newJcdsFilesCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:privileges": "Create Jamf Cloud Distribution Service Files", "jamf:api": "pro"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

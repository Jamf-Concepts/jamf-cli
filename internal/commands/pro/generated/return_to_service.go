...
// NewReturnToServiceCmd creates the return-to-service command group
func NewReturnToServiceCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "return-to-service",
		Short:       "Manage return-to-service",
		Long:        `Manage return-to-service in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newReturnToServiceListCmd(ctx))
	cmd.AddCommand(newReturnToServiceGetCmd(ctx))
	cmd.AddCommand(newReturnToServiceCreateCmd(ctx))
	cmd.AddCommand(newReturnToServiceUpdateCmd(ctx))
	cmd.AddCommand(newReturnToServiceDeleteCmd(ctx))
	cmd.AddCommand(newReturnToServiceApplyCmd(ctx))

	return cmd
}

func newReturnToServiceListCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all return-to-service
  jamf-cli pro return-to-service list

  # List return-to-service and extract IDs
  jamf-cli pro return-to-service list --field id`,
...
func newReturnToServiceGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagName string
	)
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get a return-to-service by ID
  jamf-cli pro return-to-service get 1

  # Get a return-to-service by name
  jamf-cli pro return-to-service get --name "Example"

  # Get a return-to-service and output as YAML
  jamf-cli pro return-to-service get 1 -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "View Return To Service Configurations", "jamf:api": "pro", "jamf:gateway-privileges": "return-to-service:read"},
...
func newReturnToServiceCreateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Show the JSON template for creating a return-to-service
  jamf-cli pro return-to-service create --scaffold

  # Create a return-to-service from JSON
...
func newReturnToServiceUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  # Replace a return-to-service from JSON
  echo '{"name":"Updated"}' | jamf-cli pro return-to-service update 1
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current return-to-service for --set: %w", err)
					}
				}
...
func newReturnToServiceDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
		fromFile   string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Delete a return-to-service (with confirmation)
  jamf-cli pro return-to-service delete 1

  # Delete by name
  jamf-cli pro return-to-service delete --name "Example" --yes

  # Delete without confirmation prompt
  jamf-cli pro return-to-service delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if rid == "" {
							return fmt.Errorf("no return-to-service found matching %q", entry)
						}
				...
				if flagDryRun {
					for _, e := range bulk {
						fmt.Fprintf(os.Stderr, "[dry-run] Would delete return-to-service %q (id: %s)\n", e.label, e.id)
					}
					return nil
				}
				if !flagYes {
					...
					fmt.Fprintf(os.Stderr, "⚠️  This will delete %d return-to-service. Type 'yes' to confirm: ", len(bulk))
					var confirm string
					fmt.Scanln(&confirm)
					if confirm != "yes" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete return-to-service %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode < 200 || resp.StatusCode >= 300 {
						fmt.Fprintf(os.Stderr, "delete return-to-service %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
					...
					fmt.Fprintf(os.Stderr, "Deleted return-to-service %q (id: %s)\n", e.label, e.id)
					okCount++
				}
				cooldown.Record(ctx.ProfileName)
				return batchDeleteError(cmd, okCount, failCount, firstErr, "return-to-service deletes")
			}
			...
			if flagName != "" {
				...
				if rid == "" {
					return fmt.Errorf("no return-to-service found with displayName %q", flagName)
				}
				resolvedID = rid
			...
			if flagDryRun {
				if resolvedByName != "" {
					...
					fmt.Fprintf(os.Stderr, "[dry-run] Would delete return-to-service %s\n", resolvedID)
				}
				return nil
			}
			if !flagYes {
				...
				if resolvedByName != "" {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete return-to-service %q (id: %s). Type 'yes' to confirm: ", resolvedByName, resolvedID)
				} else {
					fmt.Fprintf(os.Stderr, "⚠️  This will delete return-to-service %s. Type 'yes' to confirm: ", resolvedID)
				}
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
...
func newReturnToServiceApplyCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		fromFile     string
		flagYes      bool
		flagDryRun   bool
	...
	cmd := &cobra.Command{
		Use:         "apply",
		Short:       "Create or replace a return-to-service by name",
		Annotations: map[string]string{"jamf:api": "pro", "jamf:gateway-privileges": "return-to-service:read,return-to-service:update"},
		Long: `Create or replace a return-to-service. Reads JSON or YAML from --from-file or stdin.

The displayName field in the input is used to check if the resource
already exists. If it does, the resource is replaced (with confirmation).
If not, a new resource is created.`,
		Example: `  # Apply a return-to-service from a JSON file
  jamf-cli pro return-to-service apply --from-file return-to-service.json

  # Apply a return-to-service from a YAML file
  jamf-cli pro return-to-service apply --from-file return-to-service.yaml

  # Apply from stdin
  cat return-to-service.json | jamf-cli pro return-to-service apply

  # Apply without replacement confirmation
  jamf-cli pro return-to-service apply --from-file return-to-service.json --yes

  # Preview what would happen
  jamf-cli pro return-to-service apply --from-file return-to-service.json --dry-run`,
  ...
  "wifiProfileId": "1"
			...
			if id == "" {
				// Not found — create
				if flagDryRun {
					fmt.Fprintf(os.Stderr, "[dry-run] Would create return-to-service %q\n", name)
					return nil
				}
				...
				defer resp.Body.Close()
				fmt.Fprintf(os.Stderr, "Created return-to-service %q\n", name)
				return ctx.Output.PrintResponse(resp)
			}

			// Found — replace
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "[dry-run] Would replace return-to-service %q (id: %s)\n", name, id)
				return nil
			}
			if !flagYes {
				if noInput {
					return fmt.Errorf("return-to-service %q already exists (id: %s); use --yes to replace when --no-input is set", name, id)
				}
				fmt.Fprintf(os.Stderr, "return-to-service %q already exists (id: %s) and will be replaced. Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			defer resp.Body.Close()
			fmt.Fprintf(os.Stderr, "Replaced return-to-service %q (id: %s)\n", name, id)
			return ctx.Output.PrintResponse(resp)
		},
	}

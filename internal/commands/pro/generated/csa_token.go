...
// NewCsaTokenCmd creates the csa token command group
func NewCsaTokenCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "token",
		Short:       "Manage csa token",
		Long:        `Manage csa token in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newCsaTokenGetCmd(ctx))
	cmd.AddCommand(newCsaTokenDeleteCmd(ctx))

	return cmd
}

func newCsaTokenGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get csa token
  jamf-cli pro csa token get

  # Get csa token and output as YAML
  jamf-cli pro csa token get -o yaml`,
...
func newCsaTokenDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
	)
	...
	cmd := &cobra.Command{
		...
		Example: `  # Delete the token (with confirmation)
  jamf-cli pro csa token delete

  # Delete without confirmation prompt
  jamf-cli pro csa token delete --yes`,

...
func newDepartmentsListCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro departments list --field id`,
		Annotations: map[string]string{"jamf:privileges": "Read Departments", "jamf:api": "pro", "jamf:gateway-privileges": "departments:read"},
...
func newDepartmentsCreateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro departments get 1 -o json | jq '.name = "Copy"' | jamf-cli pro departments create`,
		Annotations: map[string]string{"jamf:privileges": "Create Departments", "jamf:api": "pro", "jamf:gateway-privileges": "departments:create"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newDepartmentsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro departments update 1 --set field=value

  # Replace a department from JSON
  ...
  jamf-cli pro departments get 1 -o json | jq '.name = "New Name"' | jamf-cli pro departments update 1`,
		Annotations: map[string]string{"jamf:privileges": "Update Departments", "jamf:api": "pro", "jamf:gateway-privileges": "departments:update"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current department for --set: %w", err)
					}
				}
				(&fieldFilter{fields: map[string]*fieldFilter{"name": nil}}).apply(current)
				setDoc, serr := buildMergePatchFromSet(flagSet, map[string]string{"name": "string"})
				if serr != nil {
					return serr
				}
...
func newDepartmentsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro departments delete 1 --yes`,
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:privileges": "Delete Departments", "jamf:api": "pro", "jamf:gateway-privileges": "departments:delete"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newDepartmentsDeleteMultipleCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  "ids": []
			...
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "Would run \"delete-multiple\" on this department\n")
				return nil
			}
			if !flagYes {
				noInput, _ := cmd.Flags().GetBool("no-input")
				...
				fmt.Fprintf(os.Stderr, "⚠️  This will run \"delete-multiple\" on this department. Type 'yes' to confirm: ")
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
...
func newDepartmentsApplyCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:api": "pro", "jamf:gateway-privileges": "departments:create,departments:read,departments:update"},

...
// NewSelfServiceCmd creates the self-service command group
func NewSelfServiceCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "self-service",
		Short:       "Manage self-service",
		Long:        `Manage self-service in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newSelfServiceGetCmd(ctx))
	cmd.AddCommand(newSelfServiceUpdateCmd(ctx))
	cmd.AddCommand(newSelfServiceHistoryCmd(ctx))
	cmd.AddCommand(newSelfServiceAddHistoryNoteCmd(ctx))
	cmd.AddCommand(newSelfServiceUploadCmd(ctx))

	return cmd
}

func newSelfServiceGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get self-service
  jamf-cli pro self-service get

  # Get self-service and output as YAML
  jamf-cli pro self-service get -o yaml`,
...
func newSelfServiceUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
		flagSet      []string
	)
  ...
  jamf-cli pro self-service update --set field=value

  # Replace self-service from a full JSON document
  jamf-cli pro self-service get -o json | jq '.field = "value"' | jamf-cli pro self-service update

  # Update from a file
  jamf-cli pro self-service update --from-file self-service.json`,
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current self-service for --set: %w", err)
					}
				}
...
func newSelfServiceHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get history for the self-service
  jamf-cli pro self-service history`,
		Annotations: map[string]string{"jamf:privileges": "Read Self Service", "jamf:api": "pro", "jamf:gateway-privileges": "self-service:read"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
...
func newSelfServiceAddHistoryNoteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
	)
...
func newSelfServiceUploadCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagFile string
	)

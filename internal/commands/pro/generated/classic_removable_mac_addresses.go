...
func newClassicRemovableMacAddressesCreateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  cat removable_mac_address.xml | jamf-cli pro classic-removable-mac-addresses create`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := ctx.Client.Do(reqCtx, "POST", "/JSSResource/removablemacaddresses/id/0", bytes.NewReader(bodyBytes))
			if err != nil {
				return err
			}
			defer resp.Body.Close()
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete removable_mac_address %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode == http.StatusNoContent || resp.StatusCode == http.StatusOK {
						...
						fmt.Fprintf(os.Stderr, "delete removable_mac_address %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
				...
				return batchDeleteError(cmd, okCount, failCount, firstErr, "removablemacaddresses deletes")
			}

...
func NewClassicRemovableMacAddressesCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd.AddCommand(newClassicRemovableMacAddressesGetByNameCmd(ctx))
	...
	cmd.AddCommand(newClassicRemovableMacAddressesDeleteByNameCmd(ctx))
...
func newClassicRemovableMacAddressesGetByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	return &cobra.Command{
		Use:   "get-by-name <name>",
		Short: "Get a removable_mac_address by name",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			path := fmt.Sprintf("/JSSResource/removablemacaddresses/name/%s", url.PathEscape(args[0]))
			resp, err := ctx.Client.Do(reqCtx, "GET", path, nil)
			if err != nil {
				return err
			}
...
func newClassicRemovableMacAddressesDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli classic-removable-mac-addresses delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flagDryRun {
				fmt.Fprintf(os.Stderr, "Would delete removable_mac_address %s\n", args[0])
				return nil
			}
			if !flagYes {
				...
				fmt.Fprintf(os.Stderr, "This will delete removable_mac_address %s. Type 'yes' to confirm: ", args[0])
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
...
func newClassicRemovableMacAddressesDeleteByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
	)
	...
	cmd := &cobra.Command{
		...
		Short: "Delete a removable_mac_address by name",
		Example: `  # Delete a removable_mac_address by name (with confirmation)
  jamf-cli classic-removable-mac-addresses delete-by-name "Example"

  # Delete without confirmation prompt
  jamf-cli classic-removable-mac-addresses delete-by-name "Example" --yes`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			name := args[0]
			...
			if id == "" {
				return fmt.Errorf("no removable_mac_address found with name %q", name)
			}
			...
			if !flagYes {
				...
				fmt.Fprintf(os.Stderr, "This will delete removable_mac_address %q (id: %s). Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")

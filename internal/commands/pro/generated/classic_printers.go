...
func newClassicPrintersListCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-printers list --field id`,
		Annotations: map[string]string{"jamf:api": "pro-classic", "jamf:gateway-privileges": "printers:read"},
...
func newClassicPrintersCreateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  cat printer.xml | jamf-cli pro classic-printers create`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := ctx.Client.Do(reqCtx, "POST", "/JSSResource/printers/id/0", bytes.NewReader(bodyBytes))
			if err != nil {
				return err
			}
			defer resp.Body.Close()
...
func newClassicPrintersDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-printers delete 1 --yes`,
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:api": "pro-classic", "jamf:gateway-privileges": "printers:delete"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			...
			if fromFile != "" {
				...
				for _, e := range bulk {
					...
					if err != nil {
						fmt.Fprintf(os.Stderr, "delete printer %q (id: %s) failed: %v\n", e.label, e.id, err)
						if firstErr == nil {
							firstErr = err
						}
						failCount++
					...
					if resp.StatusCode == http.StatusNoContent || resp.StatusCode == http.StatusOK {
						...
						fmt.Fprintf(os.Stderr, "delete printer %q (id: %s) failed: HTTP %d\n", e.label, e.id, resp.StatusCode)
						if firstErr == nil {
				...
				return batchDeleteError(cmd, okCount, failCount, firstErr, "printers deletes")
			}

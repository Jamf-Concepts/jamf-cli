...
// NewJamfConnectCmd creates the jamf-connect command group
func NewJamfConnectCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "jamf-connect",
		Short:       "Manage jamf-connect",
		Long:        `Manage jamf-connect in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newJamfConnectListCmd(ctx))
	cmd.AddCommand(newJamfConnectCreateCmd(ctx))
	cmd.AddCommand(newJamfConnectHistoryCmd(ctx))
	cmd.AddCommand(newJamfConnectAddHistoryNoteCmd(ctx))
	cmd.AddCommand(newJamfConnectTasksCmd(ctx))

	return cmd
}

func newJamfConnectListCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # List all jamf-connect
  jamf-cli pro jamf-connect list

  # List jamf-connect and extract IDs
  jamf-cli pro jamf-connect list --field id`,
...
func newJamfConnectCreateCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Show the JSON template for creating a jamf-connect
  jamf-cli pro jamf-connect create --scaffold

  # Create a jamf-connect from JSON
  echo '{"name":"Example"}' | jamf-cli pro jamf-connect create 1`,
...
func newJamfConnectHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
  ...
  jamf-cli pro jamf-connect history`,
...
func newJamfConnectTasksCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flagAll && flagPage == 0 {
				...
				for {
					// Build page-specific query
					pagePath := "/v1/jamf-connect/deployments/{id}/tasks"

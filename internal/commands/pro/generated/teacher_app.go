...
// NewTeacherAppCmd creates the teacher-app command group
func NewTeacherAppCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "teacher-app",
		Short:       "Manage teacher-app",
		Long:        `Manage teacher-app in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newTeacherAppGetCmd(ctx))
	cmd.AddCommand(newTeacherAppUpdateCmd(ctx))
	cmd.AddCommand(newTeacherAppHistoryCmd(ctx))
	cmd.AddCommand(newTeacherAppAddHistoryNoteCmd(ctx))

	return cmd
}

func newTeacherAppGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get teacher-app
  jamf-cli pro teacher-app get

  # Get teacher-app and output as YAML
  jamf-cli pro teacher-app get -o yaml`,
...
func newTeacherAppUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  # Replace teacher-app from a full JSON document
  jamf-cli pro teacher-app get -o json | jq '.field = "value"' | jamf-cli pro teacher-app update

  # Update from a file
  jamf-cli pro teacher-app update --from-file teacher-app.json`,
		Annotations: map[string]string{"jamf:privileges": "Update Teacher App Settings", "jamf:api": "pro", "jamf:gateway-privileges": "teacher-app:update"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current teacher-app for --set: %w", err)
					}
				}
				(&fieldFilter{fields: map[string]*fieldFilter{"autoClear": nil, "isEnabled": nil, "maxRestrictionLengthSeconds": nil, "safelistedApps": nil, "timezoneId": nil}}).apply(current)
				setDoc, serr := buildMergePatchFromSet(flagSet, map[string]string{"autoClear": "string", "isEnabled": "boolean", "maxRestrictionLengthSeconds": "integer", "safelistedApps": "array", "timezoneId": "string"})
				if serr != nil {
					return serr
				}
...
func newTeacherAppHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
	...
	cmd := &cobra.Command{
		...
		Example: `  # Get history for the teacher-app
  jamf-cli pro teacher-app history`,
		Annotations: map[string]string{"jamf:privileges": "Read Teacher App Settings", "jamf:api": "pro", "jamf:gateway-privileges": "teacher-app:read"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

			// Build request path
			path := "/v1/teacher-app/history"
			...
			if flagAll && flagPage == 0 {
				...
				for {
					...
					pagePath := "/v1/teacher-app/history"

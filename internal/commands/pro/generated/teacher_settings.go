...
func NewTeacherSettingsCmd(ctx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "teacher-settings",
		Short:       "Manage teacher-settings",
		Long:        `Manage teacher-settings in Jamf Pro.`,
		Annotations: map[string]string{"jamf:api": "pro"},
	}

	cmd.AddCommand(newTeacherSettingsGetCmd(ctx))
	cmd.AddCommand(newTeacherSettingsUpdateCmd(ctx))
	cmd.AddCommand(newTeacherSettingsHistoryCmd(ctx))
	cmd.AddCommand(newTeacherSettingsAddHistoryNoteCmd(ctx))

	return cmd
}

func newTeacherSettingsGetCmd(ctx *registry.CLIContext) *cobra.Command {
	var ()
  ...
  jamf-cli pro teacher-settings get -o yaml`,
		Annotations: map[string]string{"jamf:privileges": "Read Teacher App Settings", "jamf:api": "pro"},
...
func newTeacherSettingsUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
		flagSet      []string
	)
  ...
  jamf-cli pro teacher-settings update --set field=value

  # Replace teacher-settings from a full JSON document
  ...
  jamf-cli pro teacher-settings update --from-file teacher-settings.json`,
		Annotations: map[string]string{"jamf:privileges": "Update Teacher App Settings", "jamf:api": "pro"},
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			...
			if len(flagSet) > 0 {
				...
				if len(existing) > 0 {
					if err := json.Unmarshal(existing, &current); err != nil {
						return fmt.Errorf("parsing current teacher-settings for --set: %w", err)
					}
				}
...
func newTeacherSettingsHistoryCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagPage     int
		flagPageSize int
		flagSort     []string
...
func newTeacherSettingsAddHistoryNoteCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagScaffold bool
	)

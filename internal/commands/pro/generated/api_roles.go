...
func newApiRolesUpdateCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  "privileges": []
			...
			if len(flagSet) > 0 {
				...
				setDoc, serr := buildMergePatchFromSet(flagSet, map[string]string{"displayName": "string", "privileges": "array"})
				if serr != nil {
					return serr
				}
...
func newApiRolesDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro api-roles delete 1 --yes`,
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:privileges": "Delete API Roles", "jamf:api": "pro", "jamf:gateway": "unserved", "jamf:gateway-basis": "unpublished", "jamf:gateway-detail": "not declared by the gateway's Jamf Pro API 11.32.0"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

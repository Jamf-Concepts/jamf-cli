...
func newSelfServiceBrandingMacosGetByNameCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli self-service-branding-macos get-by-name "Example" -o yaml`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			id, err := resolveNameToID(reqCtx, ctx.Client, "/v1/self-service/branding/macos", "name", args[0])
			if err != nil {
				return err
			}
...
func newSelfServiceBrandingMacosDeleteByNameCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli self-service-branding-macos delete-by-name "Example" --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			noInput, _ := cmd.Flags().GetBool("no-input")
			id, err := resolveNameToIDForApply(reqCtx, ctx.Client, "/v1/self-service/branding/macos", "name", name, noInput)
			if err != nil {
				return err
			}
			if id == "" {
				return fmt.Errorf("no self-service-branding-macos found with name %q", name)
			}

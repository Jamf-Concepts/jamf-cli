...
func NewSelfServiceBrandingMacosCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd.AddCommand(newSelfServiceBrandingMacosGetByNameCmd(ctx))
	cmd.AddCommand(newSelfServiceBrandingMacosApplyCmd(ctx))
	cmd.AddCommand(newSelfServiceBrandingMacosDeleteByNameCmd(ctx))
...
func newSelfServiceBrandingMacosGetCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli self-service-branding-macos get-by-name "Example"
...
func newSelfServiceBrandingMacosGetByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	return &cobra.Command{
		Use:   "get-by-name <name>",
		Short: "Get a self-service-branding-macos by name",
		Example: `  # Get a self-service-branding-macos by name
  jamf-cli self-service-branding-macos get-by-name "Example"

  # Get by name and output as YAML
  jamf-cli self-service-branding-macos get-by-name "Example" -o yaml`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			id, err := resolveNameToID(reqCtx, ctx.Client, "/v1/self-service/branding/macos", "brandingName", "id", args[0])
			if err != nil {
				return err
			}
			path := strings.Replace("/v1/self-service/branding/macos/{id}", "{id}", url.PathEscape(id), 1)
			resp, err := ctx.Client.Do(reqCtx, "GET", path, nil)
			if err != nil {
				return err
			}
...
func newSelfServiceBrandingMacosDeleteByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
	)
	...
	cmd := &cobra.Command{
		...
		Short: "Delete a self-service-branding-macos by name",
		Example: `  # Delete a self-service-branding-macos by name (with confirmation)
  jamf-cli self-service-branding-macos delete-by-name "Example"

  # Delete without confirmation prompt
  jamf-cli self-service-branding-macos delete-by-name "Example" --yes`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			name := args[0]
			...
			if id == "" {
				return fmt.Errorf("no self-service-branding-macos found with brandingName %q", name)
			}
			...
			if !flagYes {
				...
				fmt.Fprintf(os.Stderr, "This will delete self-service-branding-macos %q (id: %s). Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")

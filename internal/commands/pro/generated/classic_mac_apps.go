...
func newClassicMacAppsDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-mac-apps delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						var resolvedID string
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "macapplications", "macapplications", entry, noInputBulk)
							if err != nil {
			...
			if flagName != "" {
				id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "macapplications", "macapplications", flagName, noInput)
				if err != nil {
					return err
				}
				if id == "" {

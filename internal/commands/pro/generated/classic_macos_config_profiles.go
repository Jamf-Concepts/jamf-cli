...
func NewClassicMacosConfigProfilesCmd(ctx *registry.CLIContext) *cobra.Command {
	...
	cmd.AddCommand(newClassicMacosConfigProfilesDeleteByNameCmd(ctx))
...
func newClassicMacosConfigProfilesDeleteByNameCmd(ctx *registry.CLIContext) *cobra.Command {
	var (
		flagYes    bool
		flagDryRun bool
	)
	...
	cmd := &cobra.Command{
		...
		Short: "Delete a os_x_configuration_profile by name",
		Example: `  # Delete a os_x_configuration_profile by name (with confirmation)
  jamf-cli classic-macos-config-profiles delete-by-name "Example"

  # Delete without confirmation prompt
  jamf-cli classic-macos-config-profiles delete-by-name "Example" --yes`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()
			name := args[0]
			...
			if id == "" {
				return fmt.Errorf("no os_x_configuration_profile found with name %q", name)
			}
			...
			if !flagYes {
				...
				fmt.Fprintf(os.Stderr, "This will delete os_x_configuration_profile %q (id: %s). Type 'yes' to confirm: ", name, id)
				var confirm string
				fmt.Scanln(&confirm)
				if confirm != "yes" {
					return fmt.Errorf("aborted")
			...
			path := fmt.Sprintf("/JSSResource/osxconfigurationprofiles/id/%s", url.PathEscape(id))
			resp, err := ctx.Client.Do(reqCtx, "DELETE", path, nil)
			if err != nil {
				return err
			}

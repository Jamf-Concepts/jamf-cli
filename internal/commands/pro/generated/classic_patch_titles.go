...
func newClassicPatchTitlesDeleteCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro classic-patch-titles delete 1 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if fromFile != "" {
				...
				for _, entry := range entries {
					if isNumericID(entry) {
						...
						if resolvedID == "" {
							id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "patchsoftwaretitles", "patchsoftwaretitles", entry, noInputBulk)
							if err != nil {
			...
			if flagName != "" {
				id, err := resolveClassicNameToIDForApply(reqCtx, ctx.Client, "patchsoftwaretitles", "patchsoftwaretitles", flagName, noInput)
				if err != nil {
					return err
				}
				if id == "" {

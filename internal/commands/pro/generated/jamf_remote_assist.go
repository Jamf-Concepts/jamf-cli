...
func newJamfRemoteAssistListCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro jamf-remote-assist list --field id`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			vft := newVersionFallback("/v2/jamf-remote-assist/session")
			...
			if flagAll && flagPage == 0 {
				...
				for {
					...
					resp, err := vft.do(ctx.Client, reqCtx, "GET", pagePath, nil, []string{"/v1/jamf-remote-assist/session"})
					if err != nil {
						return err
					}
			...
			// Make request
			resp, err := vft.do(ctx.Client, reqCtx, "GET", path, nil, []string{"/v1/jamf-remote-assist/session"})
			if err != nil {
				return err
			}
...
func newJamfRemoteAssistGetCmd(ctx *registry.CLIContext) *cobra.Command {
  ...
  jamf-cli pro jamf-remote-assist get 1 -o yaml`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			vft := newVersionFallback("/v2/jamf-remote-assist/session/{id}")

			// Make request
			resp, err := vft.do(ctx.Client, reqCtx, "GET", path, nil, []string{"/v1/jamf-remote-assist/session/{id}"})
			if err != nil {
				return err
			}
			defer resp.Body.Close()

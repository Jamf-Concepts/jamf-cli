...
func newDevicesDeleteCmd(cliCtx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:privileges": "destructive-device-actions:execute", "jamf:api": "platform-gateway", "jamf:scopes": "environment"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := platform.RequirePlatformClient(cliCtx.PlatformSDKClient); err != nil {
				return err
...
func newDevicesPatchCmd(cliCtx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:privileges": "devices:update", "jamf:api": "platform-gateway", "jamf:scopes": "environment"},
		Args:        cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if scaffoldFlag {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for page := 0; ; page++ {
				...
				if page+1 >= maxPages {
					return fmt.Errorf("applications: exceeded %d pages without reaching the end; the server may not be honoring the page parameter", maxPages)
				}
			}
			b, err := json.MarshalIndent(aggregated, "", "  ")
			if err != nil {

...
func newDeviceActionsCheckInCmd(cliCtx *registry.CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:privileges": "device-actions:execute", "jamf:api": "platform-gateway", "jamf:scopes": "environment"},
...
func newDeviceActionsRestartCmd(cliCtx *registry.CLIContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"jamf:destructive": "true", "jamf:privileges": "device-actions:execute", "jamf:api": "platform-gateway", "jamf:scopes": "environment"},
		Args:        cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := platform.RequirePlatformClient(cliCtx.PlatformSDKClient); err != nil {
				return err

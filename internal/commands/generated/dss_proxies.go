// Code generated by jamfpro-cli generator. DO NOT EDIT.
package generated

import (
	"net/url"
	"strings"

	"github.com/spf13/cobra"
)

// NewDssProxiesCmd creates the dss-proxies command group
func NewDssProxiesCmd(ctx *CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "dss-proxies",
		Short: "Manage dss-proxies",
		Long:  `Manage dss-proxies in Jamf Pro.`,
	}

	cmd.AddCommand(newDssProxiesGetCmd(ctx))

	return cmd
}

func newDssProxiesGetCmd(ctx *CLIContext) *cobra.Command {
	var ()

	cmd := &cobra.Command{
		Use:   "get <id>",
		Short: "Retrieve an existing declaration",
		Long:  "Retrieves a stored declaration based on the provided declaration id",
		Example: `  # Get a dss-proxie by ID
  jamfpro-cli dss-proxies get 1

  # Get a dss-proxie and output as YAML
  jamfpro-cli dss-proxies get 1 -o yaml`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := cmd.Context()

			// Build request path
			path := "/v1/dss-declarations/{declarationId}"
			path = strings.Replace(path, "{declarationId}", url.PathEscape(args[0]), 1)

			// Build query string
			var queryParts []string
			if len(queryParts) > 0 {
				path = path + "?" + strings.Join(queryParts, "&")
			}

			// Make request
			resp, err := ctx.Client.Do(reqCtx, "GET", path, nil)
			if err != nil {
				return err
			}
			defer resp.Body.Close()

			return ctx.Output.PrintResponse(resp)
		},
	}

	return cmd
}

// Code generated by jamfpro-cli generator. DO NOT EDIT.
package generated

import (
	"context"
	"fmt"
	"io"
	"os"
	"strings"

	"github.com/spf13/cobra"
)

// NewSelfServicePlussCmd creates the self-service-pluss command group
func NewSelfServicePlussCmd(ctx *CLIContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "self-service-pluss",
		Short: "Manage self-service-pluss",
		Long:  `Manage self-service-pluss in Jamf Pro.`,
	}

	cmd.AddCommand(newSelfServicePlussListCmd(ctx))
	cmd.AddCommand(newSelfServicePlussUpdateCmd(ctx))

	return cmd
}

func newSelfServicePlussListCmd(ctx *CLIContext) *cobra.Command {
	var ()

	cmd := &cobra.Command{
		Use:   "list",
		Short: "Determines if Self Service Plus feature toggle is enabled.",
		Long:  "This endpoint is used to determine if the Self Service Plus feature toggle is enabled.",
		Example: `  # List all self-service-pluss
  jamfpro-cli self-service-pluss list

  # List self-service-pluss and extract IDs
  jamfpro-cli self-service-pluss list --field id`,
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := context.Background()

			// Build request path
			path := "/v1/self-service-plus/feature-toggle/enabled"

			// Build query string
			var queryParts []string
			if len(queryParts) > 0 {
				path = path + "?" + strings.Join(queryParts, "&")
			}

			// Make request
			resp, err := ctx.Client.Do(reqCtx, "GET", path, nil)
			if err != nil {
				return err
			}
			defer resp.Body.Close()

			return ctx.Output.PrintResponse(resp)
		},
	}

	return cmd
}

func newSelfServicePlussUpdateCmd(ctx *CLIContext) *cobra.Command {
	var (
		flagScaffold bool
	)

	cmd := &cobra.Command{
		Use:   "update",
		Short: "Save Self Service Plus settings.",
		Long:  "Save Self Service Plus settings.",
		Example: `  # Update a self-service-plus from JSON
  echo '{"name":"Updated"}' | jamfpro-cli self-service-pluss update 1

  # Get a self-service-plus, modify, and update
  jamfpro-cli self-service-pluss get 1 -o json | jq '.name = "New Name"' | jamfpro-cli self-service-pluss update 1`,
		RunE: func(cmd *cobra.Command, args []string) error {
			reqCtx := context.Background()

			if flagScaffold {
				fmt.Println(`{
  "enabled": true
}`)
				return nil
			}

			// Build request path
			path := "/v1/self-service-plus/settings"

			// Build query string
			var queryParts []string
			if len(queryParts) > 0 {
				path = path + "?" + strings.Join(queryParts, "&")
			}

			// Make request
			// Read body from stdin if available
			var body io.Reader
			stat, _ := os.Stdin.Stat()
			if (stat.Mode() & os.ModeCharDevice) == 0 {
				body = os.Stdin
			}
			resp, err := ctx.Client.Do(reqCtx, "PUT", path, body)
			if err != nil {
				return err
			}
			defer resp.Body.Close()

			return ctx.Output.PrintResponse(resp)
		},
	}

	cmd.Flags().BoolVar(&flagScaffold, "scaffold", false, "Print a JSON template for the request body and exit")

	return cmd
}

...
func TestRunOverview_FullMock(t *testing.T) {
	...
	mock := &overviewMockClient{
		responses: map[string]overviewMockResponse{
			...
			"/v2/mdm/commands?filter=status%3D%3DPending%3BclientType%3D%3DCOMPUTER&page-size=1":      {200, `{"totalCount":12,"results":[]}`},
			"/v2/mdm/commands?filter=status%3D%3DPending%3BclientType%3D%3DMOBILE_DEVICE&page-size=1": {200, `{"totalCount":3,"results":[]}`},
			"/v2/mdm/commands?filter=status%3D%3DError&page-size=1":                                   {200, `{"totalCount":0,"results":[]}`},

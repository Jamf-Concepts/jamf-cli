...
2. **Start with the broadest useful command.** `jamf-cli overview` gives a quick instance snapshot. `jamf-cli commands -o json` lists all available commands.
...
5. **Use `--field` to extract specific values.** For example: `jamf-cli computers list -o json --field id` to get just IDs.
...
1. Check if the policy/profile exists: `jamf-cli classic-policies list -o json | ...`
2. Check its scope: `jamf-cli classic-policies get --id <id> -o json`
3. Check device group membership: `jamf-cli computer-groups list -o json`
4. Check device compliance: `jamf-cli audit --checks compliance -o json`
...
1. `jamf-cli report patch-status -o table`

### "Show me devices that haven't checked in"
1. `jamf-cli report device-compliance --days-since-checkin 14 -o table`

### "Give me an instance health check"
1. `jamf-cli overview`
2. `jamf-cli audit -o table`
...
# List all computers
jamf-cli computers list -o json

# Get specific computer
jamf-cli computers get --id 42 -o json

# List policies (Classic API)
jamf-cli classic-policies list -o json

# Get policy detail
jamf-cli classic-policies get --id 10 -o json
...
# Group tools
jamf-cli group-tools list --empty -o table
jamf-cli group-tools analyze --unused -o json
```

...
**Most platform mutations are not audited upstream at all**, and that is not something this repo can fix. Only three prod api-products declare `path_glob` rules — `securitycloud`, `blueprint-management`, `ai-policy-builder` — so the 18 mutating requests the CLI sends to blueprints, devices, device-groups, device-actions and compliance-benchmarks match no rule because no rule exists. `jamf-pro` (both `/api/pro` and `/api/proclassic`) has no audit block either, which is why moving Pro and Classic onto headers could not have cost audit coverage.

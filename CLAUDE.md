...
- `x-jamf-tenant-path-version` (spec root) — the URL version segment a spec's paths need but omit. The versionless form answers **403 `BAD_PERMISSIONS`**. **No spec sets it any more**: categories moved the version into its own paths in build v1353 and dns and ztna followed in v1416, so all five Security Cloud specs now carry their own `/v1/` prefix. The support stays because the next spec to arrive without a prefix needs it, and every ingest that dropped it was a no-op here — the generated request path is identical either way, since the version is derived from the path when the key is absent. A spec that gains the prefix while *keeping* the extension would send `/v1/v1/…`, so check the paths rather than the extension when a spec is bumped.
...
**Security Cloud URLs put the tenant *before* the version** — `/api/securitycloud/tenant/{tenantId}/v1/dns/zones`, not the version-first shape every other Jamf namespace uses. Both are routed (the Tyk definition is a catch-all proxy, wire-verified 200 for every family in both shapes), so this is not about reaching the endpoint: the gateway's **audit** rules are path globs of the form `/**/v{n}/{service}/…`, which match only when the tenant precedes the version. Version-first meant most Security Cloud mutations executed and were never recorded — a `security dns-zones delete` did the deletion and left nothing an investigation could find. `tenantBeforeVersion` in `generator/parser/platform.go` applies it, gated on `tenantFirstServices`; it moves the tenant *to* the version rather than swapping the two, because uem-connect is already tenant-first with its version after the sub-namespace and must be left alone. Note the SDK fixing this in `TenantPrefix` does **not** fix it here — generated commands hold a literal path and only substitute `{tenantId}` — so the two rules are separate copies, kept honest by `TestValidatePlatformGatewayCredentials_SecurityCloudTenant` registering the SDK-built probe path exactly rather than as a prefix. See `docs/solutions/logic-errors/securitycloud-tenant-first-url-audit-2026-08-21.md`.

**Tenant IDs are per-namespace.** Security Cloud is a separate product with its own tenant identifier, so a customer holding both has one for Jamf Pro and another for Security Cloud, both reachable from one platform profile. Configure via `security-cloud-tenant-id` in the profile, `JAMFSECURITY_TENANT_ID`, `platform setup`, or `config add-profile --security-cloud-tenant-id`; generated commands resolve it through `Transport().TenantIDFor(<service>)`. Wrong tenant → **403 `OWNERSHIP_FORBIDDEN`** (distinct from `BAD_PERMISSIONS`, which means the gateway does not route the path at all). When unset, Security Cloud paths fall back to the Pro tenant, which is right only where the two match.
...
- **`href` is always `null` through the CLI**, and that is a gateway bug rather than a CLI one: the gateway drops the `Location` header and nulls `href` whenever the response is **gzipped**, and returns both when it is not. 3/3 each way on `ztna/apps` (curl `--compressed` vs plain, 2026-08-24), matching the SDK's 12/12. Go's `net/http` sends `Accept-Encoding: gzip` on every request and decompresses transparently, so every create through this CLI sees `null` while the same call in curl shows the path. Use `id`; build nothing on `href`.

...
- `x-jamf-tenant-path-version` (spec root) — the URL version segment dns/ztna/categories paths need but omit. The versionless form the specs declare answers **403 `BAD_PERMISSIONS`**.
...
**Known upstream gaps** (wire-probed 2026-08-18, tenant `wisconsam`):

- `GET /v2/.../groups` is not routed (403 `BAD_PERMISSIONS`) — `device-groups list-v2` is the successor sitting ready, not a working command.
...
- Pagination is under-declared. Ten list ops return a paged envelope (`totalCount` + `results`) but only `sync/runs` declares `page`/`page-size`. On the wire only `ztna/apps` honours them; `categories`, `ztna/predefined-apps` and `ztna/shared-gateways` demonstrably ignore both and return everything. `ConnectorPage` and `SyncRunPage` are structurally identical yet only the latter declares the params — the tell that this is an omission.
- `dns-search-domains get` answers 404 `SEARCH_DOMAIN_NOT_SET` when no search domain is configured — an empty state rendered as an error. A candidate for `documentedStatusResults`-style handling.
- `--scaffold` renders arrays as `[]` with no element shape, so `dns-zones create --scaffold` gives no hint that `nameServers` holds `{gatewayId, ip}` objects.

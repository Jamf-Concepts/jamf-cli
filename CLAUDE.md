...
- `PUT /v1/.../groups/{groupId}` answers **200 with the updated group**, but the spec describes a body only under 204, so `successStatus` finds no schema for the status it is told to expect and the generated `update` discards what the server returned — a successful update prints nothing. Fixing this belongs in the SDK's `tools/generate/config.json` (declare the 200 with the group schema), not here; the CLI cannot optimistically decode instead, because the transport errors on an empty body when handed a non-nil result.
- `--scaffold` renders arrays as `[]` with no element shape. Confirmed still true and genuinely costly: `dns-custom-hostname-mappings update --scaffold` prints `[]`, and the element is `{hostname, aRecords[], aaaaRecords[], secureDns, ztna}` — the only feedback on a wrong guess is `400 [INVALID_FIELD] [0].aRecords: Invalid field value`. Fixing it means threading array item schemas through `parser.Property`, which changes scaffold output for all 174 generated Pro resources too, so it is deliberately left out of a Security Cloud spec bump.

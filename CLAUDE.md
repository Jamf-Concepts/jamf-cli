...
**One tenant per profile.** Jamf Pro and Jamf Security Cloud are separate products with separate tenant identifiers, and a customer holding both makes **a profile each** — `-p acme-pro`, `-p acme-jsc`. That is not a limitation to route around: the tenant travels as a per-client `X-Tenant-Id` header, so one client carries one tenant, and no command reaches across the two namespaces anyway (checked before collapsing this: every generated file references exactly one client). The pair of clients that briefly existed on this branch bought nothing, so `resolveTenantID` and a single `cliCtx.PlatformSDKClient` replaced it — with no compatibility shim, because `security-cloud-tenant-id`, `JAMFSECURITY_TENANT_ID` and `--security-cloud-tenant-id` never reached `main` and so have no users to keep working. `platform setup` asks for one tenant ID, exactly as it did before. Environment IDs will span product namespaces when they land, and `tenant-id` is the field they replace — which is the other reason not to build per-namespace tenancy on top of a field that is going away.

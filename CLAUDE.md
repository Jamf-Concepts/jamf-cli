...
- `x-jamf-tenant-path-version` (spec root) — the URL version segment a spec's paths need but omit. The versionless form answers **403 `BAD_PERMISSIONS`**. Set by dns and ztna. **Not** by categories any more: build v1353 moved the version into the categories paths themselves (`/v1/tenant/{tenantId}/categories`), so the extension was dropped in the same change. Both forms produce the same request path here, so the ingest was a no-op — but a spec that gains the prefix while keeping the extension would send `/v1/v1/…`, so check the paths, not the extension, when a spec is bumped.

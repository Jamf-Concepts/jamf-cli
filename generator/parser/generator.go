...
func needsFmt(ops []*Operation) bool {
	...
	return hasDestructive(ops) || hasQueryParams(ops) || hasDelete(ops) || hasScaffold(ops) || hasApply(ops)
}
...
{{- if needsURL .Operations }}
	"net/url"
{{- end }}
{{- if or (hasPostOrPut .Operations) (hasDestructive .Operations) (hasDelete .Operations) (hasApply .Operations) }}
	"os"
{{- end }}
	"strings"
...
If not, a new resource is created.` + "`" + `,
  ...
  jamf-cli {{ .Name }} apply --from-file {{ .NameSingular }}.json --dry-run` + "`" + `,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			noInput, _ := cmd.Flags().GetBool("no-input")
			id, err := resolveNameToIDForApply(reqCtx, ctx.Client, "{{ createPath .Operations }}", "{{ .NameField }}", name, noInput)
			if err != nil {
				return err
			}
